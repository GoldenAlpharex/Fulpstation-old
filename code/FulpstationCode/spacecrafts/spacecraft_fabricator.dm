// Me? Taking code from some other machine? No clue what you're on about. - Golden, October 2020
/obj/machinery/spacecraft_part_fabricator
	icon = 'icons/obj/robotics.dmi'
	icon_state = "fab-idle"
	name = "spacecraft fabricator"
	...
	circuit = /obj/item/circuitboard/machine/spacefab
	...
	/// New - Research of the whole station should(?) be stored in here.
	var/datum/techweb/host_research

	/// Copy of the currently synced techweb.
	var/datum/techweb/specialized/autounlocking/spacefab/stored_research
	...
	/// A list of categories that valid SPACEFAB design datums will broadly categorise themselves under.
...
/obj/machinery/spacecraft_part_fabricator/Initialize(mapload)
	stored_research = new
	host_research = SSresearch.science_tech
	rmat = AddComponent(/datum/component/remote_materials, "spacecraft_fabricator", mapload && link_on_init)
	RefreshParts() //Recalculating local material sizes if the fab isn't linked
	sync()
	return ..()

/obj/machinery/spacecraft_part_fabricator/RefreshParts()
	var/T = 0
...
/obj/machinery/spacecraft_part_fabricator/examine(mob/user)
	. = ..()
...
/**
  ...
  */
/obj/machinery/spacecraft_part_fabricator/proc/output_part_info(datum/design/D, var/categories = FALSE)
	var/cost = list()
	...
	// Start with checking if this design builds a stock part, and if so, places it in the right category.
		if(built_item in typesof(/obj/item/stock_parts/))
			var/obj/item/stock_parts/stock_part = built_item
			sub_category = list()
			if(stock_part in typesof(/obj/item/stock_parts/engine))
				sub_category += "Engines"
			if(stock_part in typesof(/obj/item/stock_parts/cell))
				sub_category += "Cells"

		//// Keeping this for reference if needed later. - Golden DELETE THIS LATER
...
/**
  ...
  */
/obj/machinery/spacecraft_part_fabricator/proc/output_available_resources()
...
/**
  ...
  */
/obj/machinery/spacecraft_part_fabricator/proc/on_start_printing()
...
/**
  ...
  */
/obj/machinery/spacecraft_part_fabricator/proc/on_finish_printing()
...
/**
  ...
  */
/obj/machinery/spacecraft_part_fabricator/proc/get_resources_w_coeff(datum/design/D)
...
/**
  ...
  * * D - Design datum to calculate the modified resource cost of.
  */
/obj/machinery/spacecraft_part_fabricator/proc/check_resources(datum/design/D)
...
/**
  ...
  */
/obj/machinery/spacecraft_part_fabricator/proc/build_next_in_queue(verbose = TRUE)
	if(!length(queue))
		return FALSE
...
/**
  ...
  * * verbose - Whether the machine should use say() procs. Set to FALSE to disable the machine saying reasons for failure to build.
  */
/obj/machinery/spacecraft_part_fabricator/proc/build_part(datum/design/D, verbose = TRUE)
	if(!D)
		return FALSE
...
/obj/machinery/spacecraft_part_fabricator/process()
...
/**
  ...
  */
/obj/machinery/spacecraft_part_fabricator/proc/dispense_built_part(datum/design/D)
...
/**
  ...
  */
/obj/machinery/spacecraft_part_fabricator/proc/add_part_set_to_queue(list/part_list)
	for(var/v in stored_research.researched_designs)
		var/datum/design/D = SSresearch.techweb_design_by_id(v)
		if((D.build_type & SPACEFAB) && (D.id in part_list))
			add_to_queue(D)
...
/**
  ...
  */
/obj/machinery/spacecraft_part_fabricator/proc/add_to_queue(datum/design/D)
	if(!istype(queue))
		queue = list()
	if(D)
		queue[++queue.len] = D
...
/**
  ...
  */
/obj/machinery/spacecraft_part_fabricator/proc/remove_from_queue(index)
...
/**
  ...
  */
/obj/machinery/spacecraft_part_fabricator/proc/list_queue()
...
/**
  ...
  *
  * Connected to the techweb via bluespace prowess - Hurray! Copies techweb research. Updates tgui's state data.
  */
/obj/machinery/spacecraft_part_fabricator/proc/sync()
...
/**
  ...
  */
/obj/machinery/spacecraft_part_fabricator/proc/get_resource_cost_w_coeff(datum/design/D, var/datum/material/resource, roundto = 1)
...
/**
  ...
  * * roundto - Rounding value for round() proc
  */
/obj/machinery/spacecraft_part_fabricator/proc/get_construction_time_w_coeff(construction_time, roundto = 1) //aran
	return round(construction_time*time_coeff, roundto)

/obj/machinery/spacecraft_part_fabricator/ui_assets(mob/user)
	return list(
		get_asset_datum(/datum/asset/spritesheet/sheetmaterials)
	)

/obj/machinery/spacecraft_part_fabricator/ui_interact(mob/user, datum/tgui/ui)
	ui = SStgui.try_update_ui(user, src, ui)
	if(!ui)
		ui = new(user, src, "SpacecraftFabricator")
		ui.open()

/obj/machinery/spacecraft_part_fabricator/ui_static_data(mob/user)
	var/list/data = list()
...
/obj/machinery/spacecraft_part_fabricator/ui_data(mob/user)
	var/list/data = list()
...
/obj/machinery/spacecraft_part_fabricator/ui_act(action, var/list/params)
	if(..())
		return TRUE
	...
	switch(action)
		...
		if("add_queue_part")
			...
			for(var/v in stored_research.researched_designs)
				var/datum/design/D = SSresearch.techweb_design_by_id(v)
				if((D.build_type & SPACEFAB) && (D.id == T))
					add_to_queue(D)
					break
			return
		if("del_queue_part")
		...
		if("build_part")
			...
			if(!(D.build_type & SPACEFAB) || !(D.id == id))
				return
...
/**
  ...
  */
/obj/machinery/spacecraft_part_fabricator/proc/eject_sheets(eject_sheet, eject_amt)
...
/obj/machinery/spacecraft_part_fabricator/proc/AfterMaterialInsert(item_inserted, id_inserted, amount_inserted)
	var/datum/material/M = item_inserted // Laugh at whoever set this to id_inserted in mech_fabricator.dm, because it should be working here and not there.
	add_overlay("fab-load-[M.name]")
	addtimer(CALLBACK(src, /atom/proc/cut_overlay, "fab-load-[M.name]"), 10)

/obj/machinery/spacecraft_part_fabricator/screwdriver_act(mob/living/user, obj/item/I)
	if(..())
		return TRUE
	if(being_built)
...
/obj/machinery/spacecraft_part_fabricator/crowbar_act(mob/living/user, obj/item/I)
	if(..())
		return TRUE
	if(being_built)
		to_chat(user, "<span class='warning'>\The [src] is currently processing! Please wait until completion.</span>")
...
/obj/machinery/spacecraft_part_fabricator/proc/is_insertion_ready(mob/user)
	if(panel_open)
...
/obj/machinery/spacecraft_part_fabricator/maint
	link_on_init = FALSE

/obj/item/circuitboard/machine/spacefab
	name = "Spacecraft Fabricator (Machine Board)"
	icon_state = "engineering"
	build_path = /obj/machinery/spacecraft_part_fabricator
	req_components = list(
...
/datum/design/board/spacefab
	name = "Machine Design (Spacecraft Fabricator Board)"
	desc = "The circuit board for a Spacecraft Fabricator."
	id = "spacecraft_fabricator"
	build_path = /obj/item/circuitboard/machine/spacefab
	category = list("Engineering Machinery")
	departmental_flags = DEPARTMENTAL_FLAG_ENGINEERING

// Research for that board is located in spacecraft.dm, with all the other spacecraft-related research.

/datum/techweb/specialized/autounlocking/spacefab
	allowed_buildtypes = SPACEFAB
